...
export const Sidebar = ({ isOpen = false, onClose = () => {} }) => {
  ...
  console.log('User from localStorage:', user.display_name);
  useEffect(() => {
    if (user) {
  ...
  return (
    <>
      ...
      <div className='flex flex-col items-center py-8 px-6 border-b border-gray-300 dark:border-gray-800'>
        ...
        <img src={logo} className='invert dark:invert-0 w-[70%] h-auto' alt="Logo" />
      </div>
          ...
          {menuItems.map((item, index) => (
            item.type === 'item' ? (
              <Link
                ...
                to={item.path}
                onClick={onClose}
                className={`flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-blue-100 dark:hover:bg-[#161617] cursor-pointer transition-colors duration-200 ${location.pathname === item.path ? 'bg-blue-100 dark:bg-[#161617] border-l-4 border-blue-600 dark:border-white' : ''
                  }`}
              >
                <Icon name={item.icon} className={`w-5 h-5 ${location.pathname === item.path ? 'text-blue-600 dark:text-white' : 'text-gray-600 dark:text-gray-300'}`} />
                <span className={`text-sm font-medium ${location.pathname === item.path ? 'text-blue-600 dark:text-white' : 'text-gray-700 dark:text-gray-300'}`}>
                  {item.name}
                </span>
              </Link>
            ) : (
              <div key={index} className='flex flex-col'>
                ...
                >
                  <Icon name={item.icon} className="w-5 h-5 text-gray-500 dark:text-gray-400" />
                  <span className='text-sm font-medium flex-1 text-gray-700 dark:text-gray-300'>{item.name}</span>
                  <svg
                  ...
                  <div className='flex flex-col ml-4 mt-1 gap-1'>
                    {item.items.map((subItem, subIndex) => (
                      <Link
                        ...
                        to={subItem.path}
                        onClick={onClose}
                        className={`flex items-center px-4 py-2 rounded-lg hover:bg-blue-100 dark:hover:bg-[#161617] cursor-pointer transition-colors duration-200 ${location.pathname === subItem.path ? 'bg-blue-100 dark:bg-[#161617] border-l-4 border-blue-600 dark:border-white' : ''
                          }`}
                      >
                        <span className={`text-sm ${location.pathname === subItem.path ? 'text-blue-600 dark:text-white' : 'text-gray-600 dark:text-gray-300'}`}>
                          {subItem.name}
                        </span>
                      </Link>
                    ))}
      ...
      <div className='relative flex-shrink-0' ref={profileMenuRef}>
        {isProfileMenuOpen && (
          <div className='absolute bottom-full left-0 right-0 mb-0 mx-2 sm:mx-4 bg-white dark:bg-[#212125] rounded-lg shadow-lg border border-gray-300 dark:border-gray-700 overflow-hidden'>
            <div className='flex flex-col'>
              <div className='px-3 sm:px-4 py-2 hover:bg-blue-100 dark:hover:bg-[#161617] cursor-pointer transition-colors duration-200' onClick={() => { navigate('/settings'); setIsProfileMenuOpen(false); }}>
        ...
        >
          <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center flex-shrink-0">
            <img src={profilePhoto} alt="" className='rounded-full w-8 h-8 sm:w-10 sm:h-10 object-cover'/>
          </div>
          <div className="flex flex-col flex-1 min-w-0">
            <div className="text-xs sm:text-sm font-medium text-gray-900 dark:text-white truncate">{profileName}</div>
            <div className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 truncate">{profileRole}</div>
          </div>
          ...
          >
            ...
            >
              {isDarkMode ? (
                <svg className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-gray-800" fill="currentColor" viewBox="0 0 20 20">
