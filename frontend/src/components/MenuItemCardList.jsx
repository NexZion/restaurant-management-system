    ...
    <div className="rounded-full bg-white/80 p-3 shadow-sm dark:bg-white/5">
      ...
      >
        <path
          ...
          strokeWidth={1.5}
          d="M12 6.5c-3.5-3-7.5-.5-7.5 3 0 4.5 7.5 8.5 7.5 8.5s7.5-4 7.5-8.5c0-3.5-4-6-7.5-3z"
        />
      </svg>
    </div>
  </div>
...
const actionIcons = {
  ...
  delete: (
    <path
      ...
      strokeLinejoin="round"
      strokeWidth={2}
      d="M4 7h16m-10 4v6m4-6v6m-7-10 1 13h8l1-13m-7-3h4l1 3H9l1-3z"
    />
  ),
  cart: (
    <path
      ...
      strokeLinejoin="round"
      strokeWidth={2}
      d="M3 4h2l2.2 10.5a2 2 0 002 1.5h7.9a2 2 0 002-1.6L20.5 8H7m3 12a1 1 0 11-2 0 1 1 0 012 0zm8 0a1 1 0 11-2 0 1 1 0 012 0z"
    />
  ),
};
  ...
  return (
    <div className={className}>
      {loading ? (
        ...
        <div className={className}>
          <div
            className={`grid ${columnClasses[safeColumns]} ${gaps[gap] || gaps.normal}`}
          >
            {visibleItems.map((item) => {
              const cartDisabled =
                disableUnavailableCart && item.available === false;
                ...
                >
                  ...
                  >
                    {!visible.image && visible.status && (
                      <div
                        className={`mb-2 flex items-center gap-1.5 text-xs font-medium ${item.available !== false ? "text-green-600 dark:text-green-400" : "text-gray-500"}`}
                      >
                        <span
                          className={`h-1.5 w-1.5 rounded-full ${item.available !== false ? "bg-green-500" : "bg-gray-400"}`}
                        />
                    ...
                    {renderExtra && (
                      <div className="mt-3">{renderExtra(item)}</div>
                    )}
