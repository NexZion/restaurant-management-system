...
export const TextField = ({ 
  ...
  return (
    <div className={`flex flex-col gap-1 ${fullWidth ? 'w-full' : ''}`}>
      ...
      {helperText && (
        <span className={`text-xs px-3 ${error ? 'text-red-500' : 'text-gray-500 dark:text-gray-400'}`}>
          {helperText}
        </span>
      )}
    </div>
...
export const ImageUploadField = ({
  ...
  return (
    <div className={`flex flex-col gap-2 items-center ${fullWidth ? 'w-full' : ''}`}>
      {label && (
        <label className={`text-sm font-medium mb-1 ${error ? 'text-red-500' : 'text-gray-900 dark:text-white'}`}>
          {label}{required && ' *'}
        </label>
      )}
      <div className="relative group" style={{ width: avatarSize, height: avatarSize }}>
        <div
        ...
        {!disabled && (
          ...
          >
            ...
            <span className="text-xs text-white font-semibold">{uploadButtonLabel}</span>
          </button>
        )}
      ...
      {helperText && (
        <span className={`text-xs text-center ${error ? 'text-red-500' : 'text-gray-500 dark:text-gray-400'}`}>
          {helperText}
        </span>
      )}
    </div>
...
export const ImageGridUploadField = ({
  ...
  return (
    <div className={`flex flex-col gap-1 ${fullWidth ? 'w-full' : ''}`}>
      {label && (
        <label className={`text-xs font-medium ${error ? 'text-red-500' : 'text-gray-600 dark:text-gray-400'}`}>
          {label}{required && ' *'}
        </label>
      )}
...
export const NumberField = ({ 
  ...
  const normalizeNumber = (rawValue) => {
    ...
    if (typeof decimals === 'number' && decimals >= 0 && decimalMatch.includes('.')) {
  ...
  const formatCommittedValue = (rawValue) => {
    ...
    if (typeof decimals === 'number' && decimals >= 0) return clamped.toFixed(decimals)
  ...
  const handleIncrement = () => {
    ...
    if (max === undefined || newValue <= max) {
      const emittedValue = typeof decimals === 'number' && decimals >= 0
        ? newValue.toFixed(decimals)
        : newValue.toString()
      emitValue(integerOnly ? String(Math.trunc(Number(emittedValue))) : emittedValue)
    }
  }
...
export const SelectField = ({ 
  ...
  const selectedOptions = multiple ? options.filter(opt => valueArray.includes(opt.value)) : []
        ...
        >
          <div className="flex-1 flex flex-wrap gap-1 items-center">
            {multiple && hasValue ? (
              ...
              <span className={`${hasValue ? 'text-gray-900 dark:text-white' : 'text-gray-400 dark:text-gray-500'}`}>
...
export const RadioField = ({
  ...
  return (
    <div className="flex flex-col gap-2">
      {/* Group Title */}
      {label && (
        <label className={`text-sm font-medium ${error ? 'text-red-500' : 'text-gray-900 dark:text-white'}`}>
          {label}{required && ' *'}
        </label>
      )}
      
      {/* Radio Options */}
      <div className={`flex ${row ? 'flex-row flex-wrap gap-4' : 'flex-col gap-2'}`}>
        {options.map((option) => {
...
export const TextAreaField = ({
  ...
  useEffect(() => {
    if (richText && editorRef.current && editorRef.current.innerHTML !== value) {
      ...
      <div className="flex items-center gap-3">
        ...
        >
          <div
            ...
            style={{ transform: checked ? currentSize.translate : 'translateX(0)' }}
          >
            {/* Icons */}
            {checked && rightIcon && (
...
export const CategoryTreeField = ({
  ...
  useLayoutEffect(() => {
    ...
    const updateSummary = () => {
      if (!selectedLabels.length) { setSummaryText(''); return }

      const availableWidth = contentEl.clientWidth
      if (!availableWidth) { setSummaryText(selectedLabels.join(', ')); return }

      const styles  = window.getComputedStyle(contentEl)
      const canvas  = document.createElement('canvas')
      const context = canvas.getContext('2d')
      if (!context) { setSummaryText(selectedLabels.join(', ')); return }

      context.font = `${styles.fontWeight} ${styles.fontSize} ${styles.fontFamily}`

      let bestText = selectedLabels.join(', ')
      for (let visibleCount = 1; visibleCount <= selectedLabels.length; visibleCount++) {
        ...
        if (context.measureText(candidateText).width <= availableWidth) bestText = candidateText
        else break
      }
      ...
      <div className="relative">
        ...
        >
          <span ref={triggerContentRef} className={`flex-1 truncate ${value.length ? 'text-gray-900 dark:text-white' : 'text-gray-400 dark:text-gray-500'}`}>
            {value.length ? (summaryText || selectedLabels.join(', ')) : placeholder}
          </span>
...
export const PhoneField = ({
  ...
  useEffect(() => {
    if (!dropOpen) return
    const close = (e) => {
      if (!triggerRef.current?.contains(e.target)) { setDropOpen(false); setSearch('') }
    }
  ...
  const selectedCountry = COUNTRY_CODES.find(c => c.dial === dialCode) ?? null
  ...
  return (
    <div className={`flex flex-col gap-1 ${fullWidth ? 'w-full' : ''}`}>
      ...
      {dropOpen && createPortal(
        <>
          <div className="fixed inset-0 z-[9998]" onClick={() => { setDropOpen(false); setSearch('') }} />
          <div
          ...
          >
            ...
            <div className="max-h-60 overflow-y-auto">
              {filtered.length === 0 ? (
                <p className="px-4 py-4 text-sm text-center text-gray-400">No results</p>
              ) : filtered.map(c => (
                <div
                  key={c.code}
                ...
                >
                  <span className="text-base w-6 text-center">
                    {c.code.toUpperCase().replace(/./g, ch => String.fromCodePoint(0x1F1E6 - 65 + ch.charCodeAt(0)))}
                  </span>
                  <span className="flex-1 truncate">{c.name}</span>
                  <span className="text-xs text-gray-500 dark:text-gray-400 font-mono">{c.dial}</span>
                </div>
              ))}
            </div>
          </div>
