...
const menuItems = [
  ...
  { id: 9, code: 'M-042', name: 'Lemonade', category: 'Beverages', description: 'Freshly squeezed lemons with soda', price: 550, available: true },
  { id: 10, code: 'M-043', name: 'Mango Smoothie', category: 'Beverages', description: 'Fresh mango blended with yogurt', price: 750, available: true },
]
...
export const POSNewOrder = () => {
  ...
  const selectedCustomer = customerOptions.find((customer) => customer.value === customerId)?.label || 'Walk-in customer'
  const selectedOrderType = orderTypes.find((type) => type.value === orderType)?.label || 'Dine in'
  ...
  const markOrderAsServed = () => {
    ...
    setPlacedOrders((current) => current.map((order) => (
      order.id === orderToServe.id ? { ...order, served: true } : order
    )))
    setOrderToServe(null)
  }

  const placedOrderRows = useMemo(() => placedOrders.map((order) => ({
    ...order,
    status: (
        ...
        <div className="grid gap-4 p-4 xl:grid-cols-[minmax(0,1fr)_450px]">
          ...
          <aside className="flex min-h-[calc(100vh-8.5rem)] flex-col overflow-hidden rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-[#18181B] xl:h-[calc(100vh-8.5rem)] xl:min-h-0">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              ...
              <Button variant="primary" disabled={!cart.length} onClick={placeOrder}>Place Order</Button>
            </div>
          </aside>
        </div>
      </div>
      ...
      >
        <div className="space-y-4">
          <div className="rounded-lg border border-blue-100 bg-blue-50 p-4 text-sm text-blue-800 dark:border-blue-500/20 dark:bg-blue-500/10 dark:text-blue-200">
            These are the orders placed during this cashier session. Mark an order as served after confirmation.
          </div>
        ...
        <div className="space-y-3">
          ...
          {orderToServe && (
            <div className="rounded-lg bg-gray-50 p-3 text-sm dark:bg-[#212125]">
              <p className="font-semibold text-gray-900 dark:text-white">{orderToServe.orderNo}</p>
              <p className="mt-1 text-gray-600 dark:text-gray-300">{orderToServe.customer}</p>
              <p className="mt-1 text-gray-500 dark:text-gray-400">{orderToServe.items}</p>
            </div>
          )}
        </div>
      </Dialog>
